...
class FacultyApplyPassScreen extends StatefulWidget {
  const FacultyApplyPassScreen({super.key});

  @override
  State<FacultyApplyPassScreen> createState() => _FacultyApplyPassScreenState();
}

class _FacultyApplyPassScreenState extends State<FacultyApplyPassScreen> {
    ...
    try {
      if (authUser != null) {
        ...
        final newPass = GatePassModel(
          ...
          status: AppConstants.statusPendingHod,
          passType: _passType,
        ...
        if (authUser.parentPhone != null && authUser.parentPhone!.isNotEmpty) {
          String msg = "CampusOne Alert: Faculty ${authUser.name} has applied for a ${_passType == AppConstants.passTypeShort ? 'Short' : 'Full Day'} Gate Pass to ${_destinationCtrl.text.trim()}. Status: Pending HOD Approval.";
          await SmsService.sendSms(phoneNumber: authUser.parentPhone!, message: msg);
        }

        // Send Push Notifications to HODs
        if (authUser.department != null) {
          final hodTokens = await fs.getHodTokens(authUser.department!);
          for (var token in hodTokens) {
      ...
      if (mounted) {
        setState(() => _isLoading = false);
        context.pop();
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Request sent to the HOD for review.')));
      }
    } catch (e) {
      if (mounted) {
        setState(() => _isLoading = false);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Apply Faculty Pass'),
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Center(
                      child: Text(
                        'This request will be sent to the HOD for review.',
